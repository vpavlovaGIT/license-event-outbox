...
public class OutboxScheduler {
    ...
    /**
     * Планировщик, который обрабатывает события из outbox.
     */
    ...
    public void processOutbox() {
        ...
        for (OutboxEvent ev : events) {
            try {
                ...
                log.info("Successfully processed event ID: {} for license: {}",
                        ev.getId(), dto.getLicenseId());

            } catch (Exception ex) {
                log.error("Failed to process event ID: {} - Payload: {} - Error: {}",
                        ev.getId(), ev.getPayload(), ex.getMessage(), ex);
            }
        }
    }
    ...
    private SoftwareLicenseDto parseJsonManually(String json) throws Exception {
        ...
        while (matcher.find()) {
            ...
            switch (field) {
                ...
                case "owner":
                    ...
                    break;
                case "expiresAt":
                    dto.setExpiresAt(LocalDate.parse(value));
                    break;
                default:
                    break;
            }
        ...
        if (dto.getLicenseId() == null || dto.getExpiresAt() == null) {
            throw new IllegalArgumentException(
                    "Invalid JSON: Missing required fields in payload: " + json
            );
        }

        log.debug("Manual parse success: licenseId={}, expiresAt={}",
                dto.getLicenseId(), dto.getExpiresAt());
